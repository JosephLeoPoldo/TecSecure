...
const int ledPin = 8; // Pin del LED verde
const int redLedPin = 9; // Pin del LED rojo
...
void setup() {
  ...
  digitalWrite(ledPin, LOW); // Apagar LED verde inicialmente

  // Configurar el pin del LED rojo como salida
  pinMode(redLedPin, OUTPUT);
  digitalWrite(redLedPin, LOW); // Apagar LED rojo inicialmente
}
...
void loop() {
  ...
  if (id > 0) {
    enviarIDMQTT(id);
    //digitalWrite(redLedPin, LOW); // Apagar LED rojo si se detectó una huella válida
  } else if (id == -2) {
    digitalWrite(redLedPin, HIGH); // Encender LED rojo si el dedo no está registrado
    delay(1000); // Mantener encendido el LED rojo por 1 segundo
    digitalWrite(redLedPin, LOW); // Apagar LED rojo después del tiempo especificado
  }
